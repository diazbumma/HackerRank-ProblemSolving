...
public class PrimeDates {
    ...
    public static void updateLeapYear(int year) {
        if(year % 400 == 0) {
            month[2] = 29; // divisible by 400 is leap year
        } else if(year % 100 == 0) {
            month[2] = 28; // divisible by 100 is common year
        } else if(year % 4 == 0) {
            month[2] = 29;
        } else {
            month[2] = 28;
    ...
    public static int findPrimeDates(int d1, int m1, int y1, int d2, int m2, int y2) {
        ...
        while(true) {
            int x = d1;
            x = x * 100 + m1;
            x = x * 10000 + y1; // one more digit for year space
            if(x % 4 == 0 || x % 7 == 0) { // using 'Or' not 'And'
                result = result + 1;
            }
            ...
            if(d1 > month[m1]) {
                ...
                if(m1 > 12) {
                    y1 =  y1 + 1;
                    m1 = 1; // month restart from january
                }
            }
        }
        return result;
